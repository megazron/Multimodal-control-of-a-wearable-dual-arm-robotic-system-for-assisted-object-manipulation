...
| **the robot MAPS, then plans against the map** | **BOTH ARMS, a VOLUME, and the wrist HOLDS STILL.** Objects are cut from the PICTURE (`segment_lift`) not clustered from the point pile, which fused two cubes 20 mm apart into one 140 mm object. The wrist no longer re-aims per cell -- ONE attitude per pass and pure translation, printer-probe style; 2 layers x 3 yaws x 2 arms = 144 cells, **70 reachable, 68 photographed (97%)**, 0 stillness refusals. Surface **-2.4 mm**; all four cubes EXACT in x,y at 41 mm against 40. `pick_from_map` is `VoxelWorld`'s first consumer ever | `test_objects_come_from_the_picture.py`, `docs/system/25_calibrate_then_plan.md` |
