...
\begin{figure}[htbp]
  \centering
  \input{figures/diagrams/kinematic_chain.tex}
...
\begin{figure}[htbp]
  ...
  \begin{subfigure}[b]{0.99\linewidth}
    \centering
    \includegraphics[width=\linewidth]{master_arm_cad_views.png}
    \caption{Four orthographic views of the assembly at the zero
    configuration, from the mechanical model.}
    \label{fig:master-cad-views}
  \end{subfigure}\\[8pt]
  \begin{subfigure}[b]{0.48\linewidth}
    \centering
    \includegraphics[width=\linewidth]{master_arm_cad_rviz.png}
    \caption{The same geometry as a kinematic description, loaded and solved
    by the robot state publisher.}
    \label{fig:master-cad-urdf}
  \end{subfigure}\hfill
  \begin{subfigure}[b]{0.48\linewidth}
    \centering
    \includegraphics[width=\linewidth]{master_arm_chain.png}
    \caption{Link lengths inferred from the model against the lengths
    measured on the built arm.}
    \label{fig:master-cad-lengths}
  \end{subfigure}
  \caption[The master arm, from model to kinematic description]{The master
  arm as designed and as described to the software. The mechanical model
  (\subref{fig:master-cad-views}) carries seven potentiometer housings, a
  shaft coupling at each joint, a cable channel along each link, and a wrist
  platform for the inertial sensor and the force-sensitive resistor; the
  cable bundle crosses every rotating joint, which is where the channel
  failures of \cref{ch:results} originate. Converting it to a kinematic
  description (\subref{fig:master-cad-urdf}) is not a formality:
  \cref{sec:cad-not-source} shows that the model and the built arm disagree
  on link length by up to \SI{9}{\milli\metre}
  (\subref{fig:master-cad-lengths}), so the measured lengths and not the
  drawn ones are what the forward kinematics uses.}
...
\begin{figure}[htbp]
  \centering
  \input{figures/diagrams/electronics.tex}
...
The driver header itself is not in the repository: it was not among the
firmware files handed over. The interface described below is therefore
established from the call sites in the sketch, which are certain, and the
register sequence is as documented for the device rather than as read from
the source. Nothing in this section depends on a line of that file that the
sketch does not call.
...
This settling figure is arithmetic on the shipped recurrence rather than a
measurement of the board. It is stated as such: the correction is derived
from the filter as written, and confirming it requires timing the settling
window on hardware, which is listed among the outstanding hardware
measurements in \cref{ch:discussion}.
...
This explanation is consistent with the firmware but has not been isolated,
and is offered as a candidate rather than as a finding. It is directly
testable without the board: \repo{scripts/virtual\_teensy.py} streams frames
in the same wire format from known ground truth, so sweeping a channel at a
known rate and counting distinct printed values against the predicted
boundary crossings would settle it. If it is confirmed, printing two decimal
places or raising the smoothing coefficient recovers the full rate.
...
The firmware as handed over and the design as described do not agree on the
force channels. The design applies a floating-point exponential average, so
those channels would carry decimal precision, and the parser on the software
side accepts a decimal in both fields. The firmware in hand transmits an
integer and applies no filter to either channel. Either the delivered build
is not the one described or the change was never made; the discrepancy is
recorded rather than resolved, because the gripper mapping does not depend on
it: the mapping applies its own deadband and hysteresis downstream.
