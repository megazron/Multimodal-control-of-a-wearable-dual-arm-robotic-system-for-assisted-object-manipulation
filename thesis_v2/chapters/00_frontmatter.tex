...
Every simulation in this thesis is run by one command and its tables are
generated from that run's own record rather than transcribed: \num{16} stages
across vision, teleoperation, the task set, shared autonomy, full autonomy
and the study's own analysis. The task set solves \num{13927}
inverse-kinematics calls over densified paths at ten repeats per pose with
zero failures and zero clearance breaches. Two results from that campaign are
new. Intent inference working from declared object coordinates becomes
\emph{confidently wrong} on half of trials once the declaration is one object
pitch out, while the same estimator working from measured coordinates stays
at zero across the whole sweep --- which states the case for measuring the
scene as a number rather than as a principle. And the replacement motion
generator removes the excursion entirely at every step size, where the
per-joint limiter it replaced settles at \SI{68}{\milli\metre} rather than
tending to zero, establishing that the excursion was a property of the
algorithm and never of its tuning.

No result reported here was validated on a physical arm carrying a load, and
no participant data was collected. The two-person study the architecture was
built for was designed in full and could not be run: the direct-teleoperation
condition it rests on depends on two master channels that a soldering fault
has left incoherent. Its analysis was therefore characterised against
constructed data instead of participants, and that characterisation is itself
a result --- the procedure recovers the effect the design was powered for on
\SI{97.3}{\percent} of \num{20000} constructed studies family-wise but on only
\SI{63.8}{\percent} for any named task once the pre-registered correction is
applied, a limitation the design's own power calculation does not expose.
\end{abstract}
