...
Twenty-five times, a result that looked like a system failure was a defect in
the measuring tool, and fourteen of those were caught only because an
independently known quantity contradicted the reading (\cref{ch:instrument}).
The base rate is high enough that any unreproduced single measurement in this
thesis should be treated as provisional. This is the reason the verification
methodology exists, the reason the task-set figures are reported with their
repeat count, and the reason several sentences in \cref{ch:results} end by
stating that a quantity has never been measured rather than offering an
estimate for it.
...
The study specified in \cref{ch:study} cannot be run today, and the blockers
are specific.
