...
\normalfont\small
This chapter is complete in structure and empty of results. Every table and
figure the study of \cref{ch:study} would produce is set out below, with the
analysis that fills it named in the caption, so that it can be populated in
the order the data arrives rather than rebuilt. The direct-teleoperation
baseline depends on master channels that are currently incoherent, and
\cref{ch:discussion} lists in priority order what must be resolved before a
session can be run.}}
\end{center}
...
\begin{table}[htbp]
  \centering
  \small
  \caption[Safety events during sessions]{Everything the safety layer did
  during the sessions, recorded automatically. This table is reported whether
  or not anything happened: a table of zeros is a result, and its absence
  would be indistinguishable from a table that was never produced.}
...
\section{Free comment}

\noindent\fbox{\parbox{0.95\linewidth}{\small\itshape
To be filled from the debrief. Comments reported by role and grouped by theme,
with the number of dyads raising each. Comments from the wearer are to be
reported at least as fully as comments from the operator: the wearer is the
participant with the least control over what happened and the most direct
experience of it.}}
