...
| **the robot MAPS, then plans against the map** | **objects are cut out of the PICTURE now, not clustered out of the point pile** -- the pre-2020 method fused two 40 mm cubes 20 mm apart into one 140 mm object and refused it. `segment_lift` (FastSAM masks lifted through depth, split by height layer, nested masks suppressed), `calibration_sweep`, `world_model`, `narration`. Scored against the renderer's own geometry: **mean 12.1 mm, worst 15.7 mm**, inside the 30 mm gate; both cubes exact in x,y. **`VoxelWorld` has its first consumer ever** -- `pick_from_map` plans a full pick and place off the measured map, 561 voxels, ran end to end. Viewing angle MEASURED (39 deg is the knee: 8/12 cells; above 72 deg nothing solves) | `test_objects_come_from_the_picture.py`, `docs/system/22_grasping.md` |
