...
| | state | check |
| --- | --- | --- |
| unit tests | 735 pass, 2 allowed failures (`test_flake8`, `test_pep257`, pre-existing: the package uses double quotes). `check_tests.py` is the gate and knows the allowlist; `python3 -m pytest -q src/*/test` is the raw run | `python3 scripts/check_tests.py` |
...
| **T1 from a typed sentence** | **works, and it does what you type.** `run_abc --instruct`, `scripts/instruct_t1.py`, or the GUI's **Instruct** tab. 75 phrasings: **34 correct / 14 asked / 27 refused / 0 MISUNDERSTOOD**, against **21 / 14 / 37 / 3** for the grammar at b782a9b on the same cases (checked out of git and run in its own interpreter). All 14 asks resolve on ONE reply, scored separately. Selectors, compounds, bare picks, politeness and misspellings; ambiguity ASKS and the ask is answerable in the same box | `scripts/sweep_t1_instructions.py` |
