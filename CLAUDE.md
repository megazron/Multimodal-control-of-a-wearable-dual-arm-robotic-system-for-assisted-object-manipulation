...
| workspace marking | **an OUTLINE now, not 192 filled tiles**, painted ON the surface (it used to be drawn at `BENCH_TOP` and floated with the objects) and clipped to it — **40 of 192 cells are dropped for having no surface under them and the count is printed**. Same set, not a bounding box: an edge wherever a cell adjoins a non-cell. 20 bars against 196 markers | `clip_scene.region_outline` |
