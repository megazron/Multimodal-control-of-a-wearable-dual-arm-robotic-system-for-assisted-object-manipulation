...
\subsection{Why a per-joint clamp cannot stay on the line}
\label{sec:clamp-maths}

The limiter it replaced applied, once per control cycle and independently to
each joint $i$,

\begin{equation}
  \label{eq:clamp}
  q_i^{(k+1)} = q_i^{(k)}
    + \operatorname{clip}\!\bigl(q_i^{\star} - q_i^{(k)},\;
                                 -\Delta,\; +\Delta\bigr),
\end{equation}

with one step bound $\Delta$ shared by all seven joints. The defect is
structural rather than a matter of tuning. Write the required joint
displacement as $\mathbf{e} = \mathbf{q}^{\star} - \mathbf{q}^{(k)}$. What the
straight line in joint space requires is that the applied step be
\emph{parallel} to $\mathbf{e}$; \cref{eq:clamp} applies
...
every component is outside it with the same magnitude. In every other case the
saturated joints advance at $\Delta$ while the unsaturated ones advance at
$e_i$, the direction of travel rotates, and the end effector leaves the
Cartesian path the inverse-kinematics solution implies. The excursion is
therefore a property of \cref{eq:clamp} and not of the value of $\Delta$,
which is why sweeping $\Delta$ makes the excursion settle at
\SI{68}{\milli\metre} rather than approach zero.
...
line between the endpoints, and that line is the entire point. The generator
solves, for each joint, the time-optimal profile subject to

\begin{equation}
  \label{eq:ruckig-limits}
  |\dot{q}_i| \le v_i^{\max}, \qquad
  |\ddot{q}_i| \le a_i^{\max}, \qquad
  |\dddot{q}_i| \le j_i^{\max},
\end{equation}

and then, rather than merely stretching each profile to the slowest joint's
duration, drives every joint from a single scalar phase
$\sigma(t) \in [0,1]$:

\begin{equation}
  \label{eq:phase-sync}
  \mathbf{q}(t) = \mathbf{q}^{(k)}
    + \sigma(t)\,\bigl(\mathbf{q}^{\star} - \mathbf{q}^{(k)}\bigr),
  \qquad \sigma(0) = 0,\; \sigma(T) = 1 .
\end{equation}

Because the same $\sigma$ multiplies the whole displacement vector, the
commanded configuration is by construction on the segment between the
endpoints for every $t$, which is exactly the property \cref{eq:clamp}
lacks. Measured on the same case: phase \SI{0.02}{\milli\metre} of path
deviation, time synchronisation \SI{9.5}{\milli\metre}, clamp
\SI{51.3}{\milli\metre}.
...
Two candidate models are distinguishable only at short travel. Writing the
commanded displacement of joint $i$ as $\delta_i = q_i^{\star} - q_i^{0}$, the
two are

\begin{equation}
  \label{eq:two-models}
  \underbrace{q_i^{\text{ach}} = q_i^{\star}
    - \varepsilon_i \operatorname{sgn}(\delta_i)}_{\text{terminal offset}},
  \qquad
  \underbrace{q_i^{\text{ach}} = q_i^{0}
    + (1 - g_i)\,\delta_i}_{\text{proportional gain error}} .
\end{equation}

They coincide when $\varepsilon_i = g_i |\delta_i|$, so at a single travel
distance no amount of data separates them; at
$|\delta| = \SI{100}{\milli\metre}$ they predict the same Cartesian error and
at \SI{20}{\milli\metre} they differ by a factor of five. Propagating the
recorded joint errors to the end effector through the manipulator Jacobian,
...
reproduces the measured Cartesian error to \SI{90}{\percent} with
$r = \num{0.923}$ under the first model and not the second, which selects the
terminal offset. A single scalar per arm,
...
\begin{figure}[htbp]
  \centering
  \input{figures/tikz/budget.tex}
  \caption[Positioning error budget]{Contributions to positioning error at
  closure, each measured separately by
  \repo{scripts/measure\_control\_budget.py}, before and after the correction
  applied to it. The camera mounting row is drawn as an open outline with no
  length: the transform between the camera frame in the kinematic description
  and the physical camera module has never been measured, there is no
  mechanical model of the module in this project, and giving that row a bar
  would be inventing a number. It is the largest remaining unknown in the
  budget and \cref{sec:hardware-session} shows it is not small.}
  \label{fig:budget}
\end{figure}

The terms are combined two ways, because which is appropriate depends on
whether they are independent. Independent contributions add in quadrature and
systematic ones add directly, so with $m$ measured terms $b_1 \dots b_m$,

\begin{equation}
  \label{eq:budget}
  \sigma_{\text{RSS}} = \sqrt{\textstyle\sum_{i} b_i^2},
  \qquad
  \sigma_{\text{worst}} = \textstyle\sum_{i} b_i ,
\end{equation}

and the honest figure to compare against the \SI{30}{\milli\metre} capture
gate is $\sigma_{\text{worst}}$, because the two largest terms are declared
constants rather than noise and therefore do not average out.

Applying the corrections moves both. The declared grasp offset is now derived
from a forward-kinematic measurement rather than asserted, taking it from
\SI{13.45}{\milli\metre} to \SI{0.05}{\milli\metre}; the shell bias is removed
by using the measured support plane; and the terminal overshoot reduces the
joint term from \SI{7.24}{\milli\metre} to \SI{1.88}{\milli\metre}. Together
these take the budget from \SI{18.64}{\milli\metre} to
\SI{12.91}{\milli\metre} in quadrature and from \SI{33.39}{\milli\metre} to
\SI{19.99}{\milli\metre} in the worst case, which is inside the requirement
for the first time --- \emph{provided} the unmeasured term is small. The pad
row is computed from the constants rather than transcribed, so reverting the
derivation makes the budget report the reversion.
