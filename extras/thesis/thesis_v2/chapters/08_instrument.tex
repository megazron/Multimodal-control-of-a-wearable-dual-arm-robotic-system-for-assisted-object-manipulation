...
\noindent\emph{\Cref{sec:validity} states the validity argument and the two
numbers that carry it. This appendix is the evidence behind it: all
twenty-six occasions on which a measurement in this project was wrong and the
system was not, what each actually was, and what revealed it.}

\label{ch:instrument}

Twenty-six times during this project a measurement was wrong and the system
was not. This chapter reports them as a body of evidence rather than as a
defect list, because the pattern across them is the most transferable outcome
of the work.
...
\Cref{tab:instr} lists the sixteen recorded during the interface work and
\cref{tab:instr2} the ten recorded afterwards, during the perception, motion
generation and hardware work. The split is itself informative: the rate did not
fall as the project matured, because each new capability brought a new
instrument with it, and the later cases are on the whole more expensive than
the earlier ones.
...
\begin{table}[htbp]
  \centering
  \caption[Instrument faults, later phases]{The nine further cases, recorded
  during the perception, motion generation and hardware work. Four of them
  produced a \emph{passing} result on a broken system, which is the more
  dangerous direction.}
  \label{tab:instr2}
  \footnotesize
  \begin{tabular}{@{}p{32mm}p{45mm}p{45mm}@{}}
    ...
    Looked like & Actually was & Caught by \\
    \midrule
    Every declared coordinate of a task unreachable
      & Every reachability sweep in the project solved at the \emph{home}
        wrist while the task layer commands a stored anchor,
        \SI{42.9}{\degree} away
      & A known-good pick reading 10 of 10 at the anchor and 0 of 10 at the
        home wrist \\
    A working surface \SI{5.2}{\milli\metre} below where it was placed
      & The reported height was the fitted plane's intercept \emph{at the
        origin}, a point inside the wearer, rather than its height over the
        working area
      & Contradiction with the declared surface height, which is exact by
        construction \\
    \SI{367.0}{\milli\metre} of clearance
      & The arm was physically against the mannequin. The model samples link
        \emph{origins}, so it cannot see the arm between joints, and it
        returns the same constant for a clear pose
      & Looking at the arm \\
    An arm reporting the saved pose to \SI{0.144}{\degree}
      & A leftover node republishing a frozen snapshot with fresh timestamps
        after its own input had died
      & The agreement being \emph{too} exact; distinct source stamps then
        settled it \\
    A test suite reporting a failing test named \texttt{=}
      & The style checker echoes the source lines it objects to, and one file
        contained the token \texttt{FAILED} inside a string literal, which the
        gate's pattern matched
      & Reading the name of the failing test \\
    A scene rendering correctly at \SI{18}{\percent} ink coverage
      & The scene was empty. The wearer alone occupies \SI{18}{\percent} of
        the frame; a populated scene reads \SIrange{46}{50}{\percent}
      & Looking at the image \\
    One task's geometry verified under another task's name
      & A conditional expression with the same operand on both branches, so
        the check read the first task's objects whichever task it was asked
        about
      & Reading the expression \\
    A jerk-limited generator \SI{0.0509}{\milli\metre} off its own path
      & The reference curve was sampled on a \SI{0.1}{\milli\metre} grid, so
        nearest-sample distance could not go below half a grid step
      & The generator being on the line to \SI{2e-16}{\radian} by construction \\
    An arm stalled \SI{360.076}{\degree} from its target
      & Three joints are continuous, so \SI{-179.05}{\degree} and
        \SI{181.02}{\degree} are the same configuration. The move had
        completed
      & The commanded and achieved poses being physically identical \\
    Every scene-camera figure recorded upside down
      & Three files decided independently whether the camera is inverted,
        two rotating and one not. None had measured it; the camera is the
        right way up (\cref{sec:orientation})
      & A second camera watching the same room, correlated against the first
...
Sorted by mechanism rather than by symptom, the twenty-six fall into four
groups. None is specific to robotics.
...
\paragraph{A constant asserted in more than one place.} The twenty-sixth case
belongs to none of the four and is worth naming separately, because it is the
one this project is most likely to repeat. Three files each carried their own
answer to ``which way up is the camera'', two of them agreeing with each other
and none of them measured. There was no single place to be wrong, so there was
no single place to check, and the disagreement between the files was silent
because no artefact was ever compared across them. The remedy is structural
rather than diligent: one owner for the fact, one function that applies it to
the image and the intrinsics together so that half of it cannot be applied,
and a stored record of the measurement that set it.
...
\begin{table}[htbp]
  \centering
  \caption{What revealed each of the twenty-six.}
  \label{tab:caught}
  \begin{tabular}{@{}lr@{}}
    \toprule
    Revealed by & Cases \\
    \midrule
    An independently known quantity contradicting the reading & \textbf{15} \\
    Inspection of the code or the method & 5 \\
    Looking at the raw artefact, usually an image & 6 \\
    \bottomrule
  \end{tabular}
\end{table}

Fourteen of twenty-six were caught because something else already had an
answer: a vendor's published stroke, a published detection score, an earlier
incompatible result, an arithmetic prediction, a declared quantity that is
exact by construction. Fifteen were caught by an independently known quantity contradicting
the reading; only five were caught by inspection, and those were the
shallowest.
...
Two practices in the later chapters follow directly from this chapter and are
worth naming, because both were adopted after a case above rather than in
advance. 	extbf{A check is required to be capable of failing on a
deliberately broken input}: the on-surface geometry check, for instance, takes
an injection argument that displaces the scene, and one instrument then
returns three different verdicts about one scene, including a pass. And
	extbf{a refusal is required to name what it refused on}, which is what
exposed a wearer-tracking gate that had never executed while its enclosing
test reported seven of seven.
...
This chapter is placed in the main body deliberately. It is a methods
contribution rather than a result, and the twenty-six cases it reports are the
reason several numbers elsewhere in this thesis are stated with the
qualifications they carry. Read as an appendix it would be a list of bugs;
read in place it is the argument for why the measurements that survived are
worth believing.
