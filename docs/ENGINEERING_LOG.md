...
| **the scene RealSense** | three files decided which way up it is, none had measured it. It is the RIGHT way up; one owner, and image and K rotate together | `findings.md` 2026-08-31 |
...
| **the master capture gates on the arm you are MOVING** | left arm, left button. Safe only because it PINS the clutch first (`force_clutch_engaged`) and REFUSES to record unless the pin confirms two ways; unpinned own-arm gating is the 2026-08-06 defect that recorded 41 of 42 segments against a still arm. **That parameter did not work** -- read once in `__init__`, so `param set` reported success and changed nothing. Live now. The e-stop needed NO bypass: both buttons held together 0.30 s since 2026-08-24; `--reset-estop` clears a stale LATCH and disables nothing | `test_the_capture_records_what_it_subscribes_to.py` |
...
| **three repairs recreated their own fault** | found only by pressing the button repeatedly from a dirty machine. Clearing shared memory restarts the daemon, and that ORPHANS THE DAEMON'S OWN SEGMENTS -- four repairs in a row. And the sweep DESTROYED A STARTING SIMULATION: a process coming up has segments it has not mapped yet, so they look unowned. Stale is now asked of the kernel (`/proc/*/maps`) with a 20 s age floor | `test_vr_bringup_sequence.py` |
...
| **the observer e-stop was unsatisfiable, and silence read as presence** | **nothing ever published `/vr/observer_estop_present`**, so the interlock was a permanent refusal somebody would route around. `scripts/observer_estop.py` is a HEARTBEAT: 5 Hz, re-confirmed every 30 s, and `vr_safety_node` treats 2 s of silence as WITHDRAWAL. It was a latch, so a publisher that stopped left the system believing somebody was standing there | `test_observer_estop_silence.py` |
...
| **the GUI button audit** | getting it to pass at all found four defects, because it had HUNG on the consent dialog since that dialog was added and never reached the presses: `Gui` had no `log` method (five buttons raised inside a Qt slot, which PyQt swallows, and did nothing silently); `procscan` was never imported, so HARD CONSTRAINT 3's own stray-`robot_state_publisher` preflight had never run; and the Launch panel rendered three of its four groups | `scripts/verify_gui_buttons.py` |
| **the pad midpoint is a CURVE, not a constant** | **the Robotiq's fingers SWING on a four-bar**: wrist-to-pad is 0.09833 m wide open, **0.10976 on a 40 mm cube**, 0.11179 on a 20 mm one. BOTH numbers this repo argued about are points on it -- the 2026-08-18 "13.47 mm too long" compared two gripper states, and T1's grasp was built 11.43 mm short. It also explains why the evidence flipped: `verify_t1` read the tips at whatever opening the SIM was left at. `pad_mid_ee_for(width_mm)` is the table; `srl_fk` walks the mimic chain now | `measure_pad_mid_ee.py --by-width` |
| **the declared pad offset** | **13.45 mm -> 0.05 mm.** `clip_tasks.PAD_OFFSET_BY_ARM` is DERIVED from the FK measurement now, so T0/T2/T3 put the pads on the coordinate they declare. It was invisible because it CANCELLED -- `clip_scene` draws each object through the same offset, so the picture was right whatever the constant was; what did not cancel is where the fingers went. Re-verified: **6024 IK calls, 0 failures** | `scripts/verify_msc_tasks.py` |
| **the shell bias reached the pick path** | `table_scene` corrected it since 2026-08-21 and `grasp_pipeline.plan_grasp` -- what `find_object` and the recorder actually plan with -- never had it. 10.5 mm in the budget, two readers of one scene. ONE `shell_corrected_centre` now; no support height means the plan SAYS the centre is the shell's; a top below the surface is REFUSED. T1 uses the plane it already gated on | `test_the_pick_path_corrects_the_shell_bias.py` |
| **the error budget** | **RSS 18.64 -> 12.91 mm, worst case 33.39 -> 19.99 mm** -- inside the 30 mm capture gate for the first time. The pad row is COMPUTED from the constants, so reverting the derivation makes the budget say so. **`camera_link` vs the physical module is still UNMEASURED and is now the largest unknown**; there is no CAD of the Kinova gripper here | `scripts/measure_control_budget.py` |
| **the verifiers asked the wrong wrist and the wrong question** | **`verify_msc_tasks` raised AttributeError ON IMPORT since T1 became two-armed -- it did not run at all.** ~20 scripts solve at the live HOME wrist, **42.9 deg from the anchor** `run_abc.send()` commands: task A's own pick reads **10/10 at the anchor, 0/10 at home**. They also asked EXACT while the follower runs a 15 deg cone -- T1 stage 1 **32 IK failures -> 0**, dance total 171 -> 68 | `verify_t1.py` |
| **T1's exact grasp is refused by T1's own table** | it solves 10/10 with no furniture and needs a **5.0 deg tilt** with the table in; the follower's cone supplies it and the tilt costs 2.|pad|.sin(tilt/2) = 8.58 mm, measured **9.58 mm** -- inside the 30 mm gate and not zero. `verify_t1` bounds the two claims separately: 2 mm at zero tilt, where the pads land BY CONSTRUCTION, and the capture gate when a tilt was needed. **OPEN** | `recordings/baselines/t1_paths.json` |
| **the clip set, RE-RECORDED 2026-08-23** | **17 cells, 8 angles.** grasp **100%**, positioning error **0.000 mm**, all five modes; T1 placement **319.7 -> 56.3 mm**; gripper motion 8/12 -> **12/12**; REAL GAPS **NONE**. Old set archived | `scripts/record_everything.sh` |
| **T2's grippers had NEVER closed** | the elastic tray was blamed and fixed; the fresh clips still read **0.000 rad for all 620-1219 samples, both arms, five modes**. `run_abc` holds a grip until the pads REACH THE OBJECT -- unsatisfiable for T2, which declares `grip_obj=None`. Applied on the spot now when there is nothing to arrive at. Re-recorded **0.0000 -> 0.5176 rad** | `test_a_grip_with_no_object_is_not_left_pending.py` |
| **three instruments reported defects that did not exist** | `accuracy_table` mapped cubes to pads with `i % 2` (single-arm row), scoring two cubes against the pad **0.58 m away**; `status_table` called T1 under four modes a REAL GAP when T1 runs under 06 ONLY; `verify_gripper_motion` demanded a RELEASE from a carry task that never releases. All three read the task's own declaration now. **And a bare `except` I wrote into the third fix did it again** | `scripts/accuracy_table.py --self-test` |
| **the robot MAPS, then plans against the map** | **BOTH ARMS, a VOLUME, the wrist HOLDS STILL, and the map is now exactly the scene: 6 objects, every one EXACT in x,y to a millimetre and every dimension within 3 mm.** Objects are cut from the PICTURE (`segment_lift`), not clustered from the point pile. One wrist attitude per pass and pure translation, printer-probe style; 2 layers x 3 yaws x 2 arms; **74 of 74 reachable cells (100%)**, 0 stillness refusals. Surface **+0.2 mm** -- the earlier '-5.2 mm' was `surface_z` reporting the plane's intercept AT THE ORIGIN, which is inside the wearer. The sweep records what it can reach and replans from it (13m15s vs 20+). `pick_from_map` is `VoxelWorld`'s first consumer ever | `test_objects_come_from_the_picture.py`, `docs/system/25_calibrate_then_plan.md` |
| **the teleop motion generator** | **it was a per-joint step clamp and it is Ruckig now, jerk-limited and synchronised.** Each joint was clamped independently, so the hand left the line the IK implies by **51.3 mm** (L) / 39.2 (R) against a 30 mm gate -- **0.0 mm** now, all seven joints arriving on ONE cycle. It never read `joint_limits.yaml`: 17.5 rad/s commanded against 1.3963, **12.53x**, now 1.00x. Not tuning: swept against its own step the excursion SETTLES at 68 mm. Velocity limits are the robot's; acceleration and jerk are ASSUMED and labelled, because the yaml declares `has_acceleration_limits: false` for all fourteen. `motion_generator:=legacy` reproduces any recording before 2026-08-23 | `scripts/measure_teleop_motion.py`, `test_motion_generator.py` (36) |
...
| **T2's tray** | **FIXED 2026-08-16.** Drawn as the LIVE `separation + 0.06`, so it ran 0.487-1.211 m against a 0.560 m spec and T2-1/T2-2 could not fail. Drawn at the spec length now, oriented along the gripper line; the ball falls off past 6.8 deg and STAYS off | `src/srl_experiments/test/test_t2_tray_is_rigid.py` |
...
| clips | the 2026-08-16 set (25 cells, three routines, eight angles, a card each) is superseded by the 2026-08-23 re-record above and archived in `extras/archive/recordings/verification_20260815_part6`, which is still the evidence for finding 3 | `scripts/status_table.py` |
...
| **the dance** | **d1 and d3 could not be filmed** until 2026-08-16: 45 of 382 and 49 of 554 waypoints unreachable, the envelope being \|x\| 0.26-0.50 against per-arm limits of 0.325 and 0.450. Moved to 0.47-0.76 as ONE symmetric band; 0 of 1298 failing | `scripts/verify_dance_paths.py` |
| **the table** | **WHITE in the picture, not just in the request**: renders 238 of 255 against the old 118. A marker's ambient is 0.5 x colour and RViz does not clamp, so the top asks for 1.88 | `scripts/probe_marker_shading.py` |
...
| **the mount caps clearance** | `base_link` sits **0.1610 m** from the torso and no joint moves it, so the 150 mm floor has 11 mm of headroom before the arm does anything. A clearance reading of exactly 0.1610 is the MOUNT | `wearer_posture.py` |
...
| **the observe pose** | **re-solved 2026-08-18 for the geometry that exists.** The old one was solved for the OLD MOUNT and the OLD cubes, and the camera looks along the tool axis. Now 12 points in frame -- both stages' six cube columns and six pad slots -- clearance 0.2202, straight transit 0.2130. **The first solve was wrong in a way the COUNT HID**: 0.335 m near, 1.17 m far, and a pad fragment passed the size gate while a real cube was rejected. Range floor 0.62 fixed it; the staged look now checks WHERE each detection is | `scripts/solve_observe_pose.py`, `scripts/stage_observe_and_detect.py` |
...
| `docs/system/VR_CHECKLIST.md` | **THE ONE TO PRINT.** One action per line, tick as you go, usable by somebody who has read nothing else. The button is `START VR TELEOP` on the RUN tab; everything else in it is what to do when that stops |
...
| `docs/system/25_calibrate_then_plan.md` | **MEASURE THE ENVIRONMENT, THEN PLAN AGAINST IT.** The sweep, the segmenter, the map, the seam to the planner, the pick, and the voice -- with the accuracy scored against the renderer's own geometry and the viewing angle measured rather than guessed |
