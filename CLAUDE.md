...
| **the verifiers asked the wrong wrist and the wrong question** | **`verify_msc_tasks` raised AttributeError ON IMPORT since T1 became two-armed -- it did not run at all.** ~20 scripts solve at the live HOME wrist, **42.9 deg from the anchor** `run_abc.send()` commands: task A's own pick reads **10/10 at the anchor, 0/10 at home**. They also asked EXACT while the follower runs a 15 deg cone -- T1 stage 1 **32 IK failures -> 0**, dance total 171 -> 68. `verify_task_scenes.task_anchor()` is the one source | `verify_t1.py`, `verify_dance_paths.py` |
...
| **the robot MAPS before it moves** | **almost everything ran on DECLARED coordinates**, which survives no real cell; and **`VoxelWorld` was constructed NOWHERE outside its own self-test**, so the planner never knew the table existed. `calibration_sweep` (straight serpentine rows), `world_model` (views -> ONE map, surface MEASURED), `narration` (15 phases, one vocabulary), `joint_planner.world_from_map()` the seam. **NOT yet wired into modes/GUI/clips** | `test_the_robot_maps_before_it_moves.py` |
