...
\addplot[only marks, mark=*, mark size=1.6pt, black]
  coordinates {(0.5,0)};
\node[font=\tiny, anchor=west, align=left] at (axis cs:0.41,0.30)
  {a tie between two candidates\\holds $P=\num{0.5}$ exactly};
\node[font=\tiny, anchor=north, red!65!black] at (axis cs:0.75,-0.005)
  {$\theta$};
\node[font=\tiny, anchor=west, align=left, red!65!black]
  at (axis cs:0.765,0.90) {assistance is orientation only;\\position is never blended};
\end{axis}
\end{tikzpicture}
