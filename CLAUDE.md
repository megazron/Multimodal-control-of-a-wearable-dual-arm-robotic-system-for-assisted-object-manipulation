...
| **the scene RealSense was UPSIDE DOWN, and no metric could see it** | three files each carried their own answer to "which way up is it": two rotated 180 deg on an assumption, the live node rotated nothing, and **nobody had measured it**. Settled against the HD webcam, which watches the same room and is rotated by nothing: un-rotated r=0.66-0.70, rotated r=0.27-0.34, over four captures. **The camera is the right way up.** A CONSISTENT rotation is an exact reflection through the optical axis, so every distance, extent and residual is preserved -- which is why nothing caught it. What did change: FastSAM 92 -> 96 regions (a CNN is not rotation-invariant) and the RANSAC plane jumped 2.68 -> 4.35 m, because its support is 3.8% and the raster order of the candidate draws reversed. `config/scene_rs_orientation.json` is the ONE owner; one function rotates the image and K together so half a rotation cannot be applied | `scripts/scene_rs_orientation.py --self-test` (12), `test_scene_rs_is_the_right_way_up.py` |
