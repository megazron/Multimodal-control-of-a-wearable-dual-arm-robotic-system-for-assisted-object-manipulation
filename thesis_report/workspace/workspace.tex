...
while the medians barely moved --- which is what establishes that the outliers
were chain compounding rather than geometry.
...
freeing the wrist yaw raises each arm's individual reach from sixteen--eighteen
...
\begin{table}[htbp]
  ...
  the 63-cell frontal grid, arms verified at home. The result is robust to
  wrist freedom and to additional inverse-kinematics retries.}
  \label{tab:overlap}
  \begin{tabular}{lccc}
    \toprule
...
\begin{figure}[htbp]
  ...
  the mounts outward \emph{hurts}. The sweep is purely kinematic --- it moves
  the arms' bases without moving the wearer --- so these are upper bounds and
  any candidate requires a clearance check before adoption.}
  \label{fig:mount}
\end{figure}
...
inverse kinematics with collision checking enabled --- the planner reports the
poses valid --- and yet the arms pass within \SIrange{48}{67}{\milli\metre} of
