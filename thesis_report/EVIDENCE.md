...
dated section headings in `CLAUDE.md`, which run 2026-07-31 to 2026-08-08 and
...
the DIVERGENCE table's milestone column must be sourced from `CLAUDE.md` with
that caveat stated.
...
reach is 0.902 m (quoted in `CLAUDE.md`). The ratio is therefore
...
robot down. Recorded in `CLAUDE.md`; the regression coefficients are quoted
...
| Frame rate | ~50 Hz | measured, `CLAUDE.md` |
| **Sensor update rate** | **14.7–17 Hz** — so 82–88 % of adjacent frames are bit-identical | measured, `CLAUDE.md` |
...
| IMU part number / mounting | **PARTIAL** | code implies an MPU-6050/9250-class device at I²C `0x68`/`0x69` (`CLAUDE.md`); not in repo source |
...
| No reach observable survives on left ($R^2=0.133$) | analysis | `CLAUDE.md`; regression not retained | PARTIAL | Medium |
...
| Dead-man trips 0.94–0.99 s (stale) / 0.97 s (silent) | **log text only** | `CLAUDE.md` | PARTIAL | Medium |
...
only in `CLAUDE.md`. *Needed:* re-run with output to file.
