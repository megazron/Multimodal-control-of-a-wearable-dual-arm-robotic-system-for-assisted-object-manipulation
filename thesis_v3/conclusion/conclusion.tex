...
no recalibration fixes it. The safety layer handled fourteen of fourteen
injected faults with every blocker observable by name. The workspace
characterisation found the two arms share no reachable point under the
as-built mount, identified a buildable mount correction, and that
...
approval with five operators across three tasks, showing shorter completion
times and fewer re-grips under assistance, at the cost of more hand travel
per session and with no measured cost in responsiveness.
...
In priority order: repairing the two identified master channels, so the
mannequin master becomes a usable direct-control baseline; calibrating the
wrist-camera-to-arm transform, the largest unmeasured budget term and the
reason no open-loop grasp has closed on hardware; correcting the clearance
check to sample link geometry rather than link origins -- the one item here
that is a safety defect rather than an incompleteness; and running the full
two-person protocol of \cref{app:study} with a separate wearer, now that
approval and a working input path both exist, to measure what this project
was ultimately built to ask: the exchange rate between what shared autonomy
buys an operator and what it costs the person carrying the arms.
