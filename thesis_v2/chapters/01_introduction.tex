...
\begin{figure}[htbp]
  \centering
  \input{figures/tikz/overview.tex}
...
\begin{enumerate}
  ...
  \item \textbf{A validation of the study's own analysis against constructed
    ...
    \emph{Measured on constructed data;} \cref{ch:participant}.
  ...
  \item \textbf{A reproducible simulation campaign.} Every simulation this
    ...
    \num{13927} inverse-kinematics calls at zero failures, and one stage that
    failed on its first pass because its own monotonicity control caught the
    solver reporting a physical impossibility. \emph{Measured;}
    \cref{sec:campaign}.
...
transferable outcome of the work, and \cref{ch:instrument} reports the whole
body of evidence rather than a selection.

\section{Thesis structure}

\Cref{ch:background} reviews the field by theme and ends with the gap this
work addresses. \Cref{ch:master} documents the master arm: its mechanical
chain, its electronics and its firmware. \Cref{ch:development} records how the
control approach reached its present form, including the routes that were
tried and abandoned. \Cref{ch:method} describes the system as built.
\Cref{ch:perception} covers the perception stage that replaced declared
coordinates with measured ones, the planner that consults it, the
natural-language route into the task layer, and the camera that observes the
wearer. \Cref{ch:vision} then reports what that pipeline does on four
physical cameras, stage by stage, and is together with
\cref{sec:hardware-session} the only part of this thesis whose numbers come
from hardware in the laboratory. \Cref{ch:motion} covers motion generation and what is known about
transferring a joint command to a physical arm, and assembles the positioning
and reaction budgets. \Cref{ch:instrument} is the methods contribution: every
occasion on which the instrument rather than the system was at fault.
\Cref{ch:results} reports what was measured, labelled throughout with the
strength of evidence behind it. \Cref{ch:workspace} presents the workspace
characterisation separately, because it is a chain of measurements whose
consequences determine the shape of the task set. \Cref{ch:study} specifies
the two-person study in full, and \cref{ch:participant} reports what can be
established about that study without participants: the analysis written out,
and a measurement of what it does to data whose answer is known. \Cref{ch:discussion} interprets the results and states the
limitations, and \cref{ch:conclusion} sets out what would most improve the
system, in the order it should be done.
