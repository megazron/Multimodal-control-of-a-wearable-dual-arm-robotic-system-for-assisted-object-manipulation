"""CLAUDE.md must stay short AND still carry every damaging constraint.
...
time. Each must survive in CLAUDE.md itself, because a fresh session reads
...
def test_it_stays_short():
    ...
    n = len(text())
    assert n < 60000, ("CLAUDE.md is %d bytes, about %dk tokens. New findings "
...
def test_every_damaging_constraint_survives():
    ...
    for phrase, why in [
        ...
        assert phrase in t, "CLAUDE.md lost a damaging constraint: %s (%s)" % (
            phrase, why)
...
def test_it_says_how_to_run_things():
    ...
    for cmd in ("ros2 run srl_teleop gui", "check_channels.sh",
        ...
        assert cmd in t, "CLAUDE.md no longer says how to: %s" % cmd
