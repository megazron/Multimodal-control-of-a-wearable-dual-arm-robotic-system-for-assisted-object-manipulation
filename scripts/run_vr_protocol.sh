#!/usr/bin/env bash
# Prompted VR teleoperation measurement, one motion at a time.
#
...
echo "results     $OUT/protocol_$STAMP.json"
echo

exec python3 "$WS/scripts/vr_teleop_protocol.py" \
    --out "$OUT/protocol_$STAMP.json" "$@"
