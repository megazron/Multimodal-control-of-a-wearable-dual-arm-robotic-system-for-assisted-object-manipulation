...
\begin{enumerate}
  ...
  \item \textbf{Converge the orientation-cost sweep.} It is the one stage of
    the campaign that its own control refuses, and the refusal is correct:
    the mean reach moved \SI{52}{\milli\metre} between three and ten seeds,
    so the number is not established. It needs no hardware and no new code,
    only a longer run, and until it is converged the cost of the pinned wrist
    is known in ordering but not in magnitude.
