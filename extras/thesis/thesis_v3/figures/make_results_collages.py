...
and figures/pilot/data/study_summary.json.
    ...
    python3 extras/thesis/thesis_v3/figures/make_results_collages.py
"""
import csv, json, os
import numpy as np
import matplotlib
...
plt.rcParams.update({"font.size": 9, "axes.titlesize": 9.5, "axes.labelsize": 9, "xtick.labelsize": 8.5,
                     "ytick.labelsize": 8.5, "legend.fontsize": 8.5, "figure.dpi": 150, "savefig.bbox": "tight",
...
def tlx():
    ...
    gs = fig.add_gridspec(3, 6, height_ratios=[1.0, 1.0, 0.95], hspace=0.6, wspace=1.1)
    # (a) RTLX per condition, each task
    for k, (tc, tl) in enumerate(TASK):
        ax = fig.add_subplot(gs[0, 0:3] if k == 0 else gs[0, 3:6])
        for i, (cc, cl) in enumerate(COND):
            ...
            xs = np.linspace(-0.2, 0.2, len(v))
            ax.scatter(i + xs, v, s=16, color=COL[cc], zorder=3)
            for r, x in zip(sel(cc, tc), xs):
                ax.text(i + x + 0.04, float(r["rtlx"]), r["participant"], fontsize=8.5, ha="left", va="center", color=GREY)
    ...
    for j, (tc, cc) in enumerate(cols):
        for i, p in enumerate(PARTS):
            ...
            ax.text(j + 0.5, i + 0.5, lab, ha="center", va="center", fontsize=8.5, color="white")
    ax.set_xlim(0, len(cols)); ax.set_ylim(len(PARTS), 0)
    ax.set_xticks(np.arange(len(cols)) + 0.5)
    ax.set_xticklabels(["%s\n%s" % ("pick+place" if tc == "PP" else "reach", {"VR": "VR", "MM": "mannequin", "SA": "shared"}[cc]) for tc, cc in cols], fontsize=8, rotation=35, ha="right", rotation_mode="anchor")
    exp = {p: [r for r in rows if r["participant"] == p][0] for p in PARTS}
    ax.set_yticks(np.arange(len(PARTS)) + 0.5)
    ax.set_yticklabels(["%s (VR %s, robot %s)" % (p, exp[p]["vr_experience"].lower(), exp[p]["robot_experience"].lower()) for p in PARTS], fontsize=8.5)
    ...
    ax.set_xticks([1, 2, 3]); ax.set_xticklabels(["novice", "inter-\nmediate", "exper-\nienced"]); ax.set_xlim(0.6, 3.4); ax.set_ylim(0, 115); ax.set_yticks(range(0, 101, 20))
    ax.set_xlabel("prior robot experience"); ax.set_ylabel("mean RTLX per operator"); ax.legend(frameon=False, loc="upper right", ncol=1, handletextpad=0.2, fontsize=7.5)
    panel_letter(ax, "(d)")
...
def dvs():
    ...
    for k, (m, lab) in enumerate(slopes):
        ...
        for p, a, b in zip(P, a_all, b_all):
            ...
            ax.text(-0.08, a, p["participant"], fontsize=8.5, ha="right", va="center", color=GREY)
        up = sum(1 for a, b in zip(a_all, b_all) if b > a)
        ax.set_xticks([0, 1]); ax.set_xticklabels(["direct", "with\nassistance"]); ax.set_xlim(-0.4, 1.25); ax.set_ylabel(lab); ax.margins(y=0.12)
        ax.set_title(["(d)", "(e)", "(f)"][k], loc="left", fontsize=9, fontweight="bold")
        ax.text(0.5, 1.0, "%d of %d higher\nwith assistance" % (up, len(P)), transform=ax.transAxes, ha="center", va="bottom", fontsize=8, color=GREY)
