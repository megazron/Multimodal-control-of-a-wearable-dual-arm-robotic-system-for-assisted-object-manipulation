# Computer vision stages, 2026-08-30T07:31:41
...
Read from `/real/joint_states` (706 distinct source stamps), compared with the saved ideal pick pose.

| arm | reference | joints read | worst deviation | at the pick pose |
| --- | --- | --- | --- | --- |
| left | `config/pick_pose_ideal_left.txt` | 7 of 7 | 10.67 deg | **NO** |
| right | `config/pick_pose_ideal_right.txt` | 7 of 7 | 13.53 deg | **NO** |

<details><summary>left, joint by joint</summary>

| joint | measured (deg) | pick pose (deg) | delta (deg) |
| --- | --- | --- | --- |
| joint_1 | 94.984 | 97.743 | -2.759 |
| joint_2 | 44.406 | 37.529 | +6.878 |
| joint_3 | 125.353 | 134.368 | -9.015 |
| joint_4 | -86.765 | -87.339 | +0.574 |
| joint_5 | -20.539 | -18.818 | -1.721 |
| joint_6 | -26.269 | -28.989 | +2.720 |
| joint_7 | -118.039 | -128.712 | +10.673 |
...
| joint | measured (deg) | pick pose (deg) | delta (deg) |
| --- | --- | --- | --- |
| joint_1 | 129.533 | 134.916 | -5.383 |
| joint_2 | -67.020 | -66.576 | -0.444 |
| joint_3 | 39.399 | 25.867 | +13.532 |
| joint_4 | -41.258 | -31.455 | -9.804 |
| joint_5 | 159.680 | 161.696 | -2.017 |
| joint_6 | 60.424 | 71.186 | -10.762 |
| joint_7 | 153.204 | 153.932 | -0.728 |
...
- **source** Kinova vision module on the left arm, over ROS topics /left_camera/{color,depth} (driver: kinova_vision_node, started by bringup_arm.sh), ROS_DOMAIN_ID=0
...
- **source** FastSAM-s.pt (~11M parameters) via ultralytics, imgsz=1024 conf=0.25 iou=0.7 on the cpu, 6.1 s for this frame
...
- **formula** `none -- this is the input every later stage is a function of`
- **source** Kinova vision module on the right arm, over ROS topics /right_camera/{color,depth} (driver: kinova_vision_node, started by bringup_arm.sh), ROS_DOMAIN_ID=0
...
- **formula** `none -- this is the input every later stage is a function of`
- **source** RealSense D435i across the room, depth 640x480@15 + colour 640x480@15, USB 2.1, negotiated against what the device reports it has. Depth is aligned to colour by librealsense and averaged over 12 frames, valid pixels only.
...
- **note** rotated 180 deg: the camera is mounted upside down and the principal point is rotated WITH the image (cx'=W-1-cx); liveness: 12 frames, 7 distinct depth images, 7 distinct frame numbers, 15% of pixels valid
...
- **formula** `object-agnostic instance masks; no class, no prompt, no scene knowledge -- 'what are the regions', not 'where is the green cube'`
- **source** FastSAM-s.pt (~11M parameters) via ultralytics, imgsz=1024 conf=0.25 iou=0.7 on the cpu, 2.1 s for this frame
- **summary** 92 regions | largest 82781 px (26.9% of frame), smallest 82 px

| quantity | value | unit |
| --- | ---: | --- |
| regions returned | 92 | count |
| largest region | 82781 | px |
| largest as a fraction of the frame | 26.95 | % |
| smallest region | 82 | px |
| median region | 597.5 | px |
...
- **source** srl_perception/segment_lift.py: MIN_AREA_PX, MAX_AREA_FRAC
- **summary** 92 in, 92 kept, 0 too small, 0 too large
...
- **formula** `for each region: mean BGR over the MASK, then the ratio test of stage 08; score = min(1, area/2000)`
- **source** srl_perception/prompt_detector.py:_segment -- the backend that actually found the target on this rig
- **summary** 179 px at (320, 196), mean BGR [81.0, 122.2, 41.4]

| quantity | value | unit |
| --- | ---: | --- |
| colour word parsed | green | - |
| regions considered | 92 | count |
| regions matched | 1 | count |
| regions rejected on colour | 91 | count |
...
- **summary** 38526 points in the 0.15-8.00 m band | extent X -3.477 to 1.141, Y -1.457 to 3.051, Z 0.317 to 7.994 m
...
- **summary** normal (-0.2732, 0.0790, -0.9587), offset 2.6822 m | 1443 of 38526 points are inliers (3.7%) | RMS 3.37 mm | 16.5 deg between the plane normal and the camera's optical axis
...
| inliers | 1443 | count |
| inlier fraction | 3.75 | % |
| residual RMS | 3.368 | mm |
| inlier tolerance | 6 | mm |
| RANSAC iterations | 400 | count |
| angle to the optical axis | 16.52 | deg |
...
- **formula** `C = cov(Q - mean(Q)) for inliers Q;  n = eigvec(C)[:, argmin];  d = -n . mean(Q)   <-- BOTH halves, together`
- **source** scripts/servo_pick_left.py:fit_plane, imported
- **summary** the normal moved 0.000 deg | RMS 3.37 mm -> 3.37 mm | mixing the refined normal with the RANSAC offset selects 1443 inliers instead of 1443

| quantity | value | unit |
| --- | ---: | --- |
| normal moved | 0 | deg |
| RMS before refinement | 3.368 | mm |
| RMS after refinement | 3.368 | mm |
| inliers with the matched pair | 1443 | count |
| inliers with a mixed normal and offset | 1443 | count |
| inliers lost by mixing | 0 | count |
...
- **formula** `h(P) = n . P + d,  signed;  positive is off the surface, toward the camera`
- **source** servo_pick_left.measure -- the same expression the live pick uses to separate the cube from the table
- **summary** 5552 of 38526 points stand more than 5 mm off the plane (14.41%) | tallest 2403.9 mm

| quantity | value | unit |
| --- | ---: | --- |
| height threshold | 5 | mm |
| points above it | 5552 | count |
| that fraction | 14.411 | % |
| tallest point | 2404 | mm |
| lowest point | -4863 | mm |
| median height | -1512 | mm |
...
- **formula** `bin at 2 mm; take the fullest bin; require it to hold >= 20% of the points; refine as the mean of everything within +/-6 mm of it`
- **source** srl_perception/surface_from_depth.py, applied along the fitted plane normal because this program has no camera-to-world transform; the node applies it to world z
- **summary** modal bin holds 0.7% of 38526 points (floor 20%) | refined -2.6816 m, spread 3.32 mm | a plain MEAN would say -3.8957 m, which is 1214.1 mm out | RANSAC's own offset is -2.6822 m, 0.59 mm from this

| quantity | value | unit |
| --- | ---: | --- |
| bin width | 2 | mm |
| points binned | 38526 | count |
| share in the fullest bin | 0.73 | % |
| share required | 20 | % |
| modal bin centre | -2.6822 | m |
| refined estimate | -2.6816 | m |
| spread of the inliers | 3.321 | mm |
| inlier band | 6 | mm |
| a plain mean would say | -3.8957 | m |
| that mean's error | 1214 | mm |
| RANSAC's own offset | -2.6822 | m |
| the two estimators differ by | 0.592 | mm |
...
- **formula** `{ pixels whose lifted point has h > 5 mm }, 8-connected`
- **source** the plane of stage 18 fitted in the colour frame; heights from the aligned depth of stage 16
- **summary** 1892 px, 1769 pts, top 75 mm | 1026 px, 988 pts, top 700 mm | 384 px, 379 pts, top 18 mm | 353 px, 343 pts, top 27 mm | 302 px, 295 pts, top 793 mm | 298 px, 290 pts, top 2404 mm

| quantity | value | unit |
| --- | ---: | --- |
| pieces standing on the surface | 18 | count |
| height threshold | 5 | mm |
| largest piece | 1892 | px |
| tallest piece | 2404 | mm |


*pieces above the plane* -- 18 rows, full data in `data/scene_rs/22_on_surface__pieces_above_the_plane.csv`
...
- **formula** `find the MODAL height bin (5 mm bins); cut 12 mm above it; recurse on what is left, twice`
- **source** srl_perception/segment_lift.py:_layers
- **summary** layer 1: 4-2404 mm, 1310 points

| quantity | value | unit |
| --- | ---: | --- |
| region area | 70421 | px |
| points lifted from it | 8027 | count |
| points above the plane | 1310 | count |
| pixels removed by the 1 px erosion | 3743 | px |
| layers found | 1 | count |
| lowest point | 4.03 | mm |
| highest point | 2404 | mm |


*height layers* -- 1 rows, full data in `data/scene_rs/24_layers__height_layers.csv`
...
- **formula** `width = min over theta of [ max(p.u(theta)) - min(p.u(theta)) ],  u(theta) = (cos, sin) in the support plane, swept at 0.25 deg`
- **source** srl_perception/segment_lift.py:_min_width and rgbd_grasp.min_width_frame
- **summary** calipers 733.0 mm | PCA's short axis 754.1 mm | axis-aligned box 950.1 mm | long axis 1575.4 mm, height 2399.8 mm | jaws close on 85 mm, so this is TOO WIDE

| quantity | value | unit |
| --- | ---: | --- |
| instance points | 1310 | count |
| minimum width, rotating calipers | 733.01 | mm |
| at this angle in the plane | 80 | deg |
| PCA short axis, THE CONTROL | 754.1 | mm |
| axis-aligned box, THE CONTROL | 950.13 | mm |
| PCA overstates by | 21.08 | mm |
| long axis | 1575 | mm |
| height | 2400 | mm |
| jaw span | 85 | mm |
| graspable as it lies | no | - |
...
- **formula** `voxelise each instance at 5 mm; accept smallest first; reject one whose claimed-voxel overlap exceeds 0.55`
- **source** srl_perception/segment_lift.py:_suppress_nested
- **summary** 18 instances in, 15 kept, 3 dropped | overlaps: 1.00, 0.86, 0.90

| quantity | value | unit |
| --- | ---: | --- |
| instances in | 18 | count |
| kept | 15 | count |
...
*per instance* -- 18 rows, full data in `data/scene_rs/26_nested__per_instance.csv`
...
- **formula** `fit the plane, keep its inliers, span them in the plane's own basis; extent taken at the 2nd and 98th percentile so one stray return cannot stretch it`
- **source** the plane of stage 18, expressed as a rectangle in the camera frame
- **summary** 0.416 x 1.413 m of surface, 2.682 m away, flat to 3.37 mm

| quantity | value | unit |
| --- | ---: | --- |
| points on the surface | 1443 | count |
| surface extent along its first axis | 0.4164 | m |
| surface extent along its second axis | 1.4126 | m |
| area spanned | 0.5882 | m2 |
| perpendicular distance from the camera | 2.6822 | m |
| distance to the middle of that extent | 2.7485 | m |
| nearest point on the surface | 2.7406 | m |
| furthest point on the surface | 3.1141 | m |
| tilt from the optical axis | 16.52 | deg |
| flatness, RMS of the inliers | 3.368 | mm |
...
- **formula** `centre = mid-extent ACROSS the surface + (foot + top/2) ALONG the normal; axes = (minimum-width direction, its perpendicular, the plane normal); size = the spans along them`
- **source** segment_lift._measure for the horizontal centre and rgbd_grasp.shell_corrected_centre's reasoning for the vertical -- the combination is checked to 2 mm against a constructed cube in --self-test
- **summary** 12 objects, 1.757 to 2.974 m away

| quantity | value | unit |
| --- | ---: | --- |
| objects posed | 12 | count |
| nearest object | 1.7571 | m |
| furthest object | 2.9735 | m |
| largest footprint | 1575 | mm |
| tallest object | 2404 | mm |
...
- **formula** `range = |centre| (the camera is the origin); height = n.c + d; separation = |c_i - c_j|; free gap subtracts each object's half-width`
- **source** the poses of stage 30, all in the camera's own frame
- **summary** 10 objects, nearest 1.757 m, closest pair 0.120 m

| quantity | value | unit |
| --- | ---: | --- |
| objects measured | 10 | count |
| nearest object | 1.7571 | m |
| furthest object | 2.8095 | m |
| closest pair | 0.1198 | m |
| tightest free gap | 0 | m |
...
- **source** MediaPipe PoseLandmarker (full), num_poses=4, run in .venv_pose as a separate interpreter -- the same model srl_perception/wearer_tracker_node.py uses, 5.1 s
- **summary** 1 person(s)
...
- **formula** `object-agnostic instance masks; no class, no prompt, no scene knowledge -- 'what are the regions', not 'where is the green cube'`
- **source** FastSAM-s.pt (~11M parameters) via ultralytics, imgsz=1024 conf=0.25 iou=0.7 on the cpu, 3.5 s for this frame
...
- **refused** YOLO-World ran in 10.6 s over classes ['green cube'] at conf 0.05 and found NOTHING. This is the measured behaviour on this rig, not a misconfiguration: it finds a person at 0.725 and a laptop at 0.739 in the same room and cannot see the small dark cube at any confidence. It is why the backend order is segment first, naming second, colour as the floor.
