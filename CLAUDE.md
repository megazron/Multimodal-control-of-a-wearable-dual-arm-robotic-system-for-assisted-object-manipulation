...
| **the pad midpoint is a CURVE, not a constant** | **the Robotiq 85's fingers SWING on a four-bar, so the wrist-to-pad distance depends on how open the hand is** -- 0.09833 m wide open, **0.10976 on a 40 mm cube**, 0.11179 on a 20 mm one. BOTH numbers this repo has argued about are points on it: `PAD_OFFSET_BY_ARM`'s 0.11178 is the hand almost SHUT and `PAD_MID_EE`'s 0.09833 is WIDE OPEN, and the 2026-08-18 "13.47 mm too long" compared two gripper states. T1's grasp was built 11.43 mm short. It also explains why the evidence flipped: `verify_t1` read the tips at whatever opening the SIM was left at -- 0.00 mm in 2026-08-18 (open), 13.52 mm today (shut), same geometry. `pad_mid_ee_for(width_mm)` is the table; `srl_fk` walks the mimic chain now, so the fingers are placeable offline at a STATED opening | `scripts/measure_pad_mid_ee.py --by-width`, `test_pad_offset_has_one_source.py` |
...
| **the verifiers asked the wrong wrist and the wrong question** | **`verify_msc_tasks` had raised AttributeError ON IMPORT since T1 became two-armed -- it did not run at all.** And ~20 scripts solve at the live HOME wrist, **42.9 deg (left) / 27.3 (right) from the anchor** `run_abc.send()` commands: task A's own pick reads **10/10 at the anchor and 0/10 at home**. They also asked EXACT while the follower has run a 15 deg cone in every mode since 2026-08-22 -- T1 stage 1 **32 IK failures -> 0**, stage 2 48/16 -> 0/0, dance d1 64 -> 0, total 171 -> 68. `verify_task_scenes.task_anchor()` is the one source; `--orientation-policy exact` reproduces every earlier figure | `scripts/verify_t1.py`, `scripts/verify_dance_paths.py` |
...
| **the clip set, RE-RECORDED 2026-08-23** | **17 cells, 8 angles each, against the fixed geometry.** grasp **100%** and positioning error **0.000 mm** in all five modes; T1 placement **319.7 -> 56.3 mm** (var 264 -> **0.59**); `verify_rviz_clips` 33/33; `verify_gripper_motion` 8/12 -> **12/12**; `status_table` REAL GAPS **NONE**. The old set is in `archive/recordings/verification_20260823_pre_accuracy_pass` | `scripts/record_everything.sh` |
| **T2's grippers had NEVER closed, and the tray was not why** | the elastic tray was blamed and fixed on 2026-08-16; the fresh clips still read **0.000 rad for every one of 620-1219 samples on both arms in all five modes**. `run_abc` holds a commanded grip until the pads REACH THE OBJECT -- right for a pick, unsatisfiable for T2, which declares `grip_obj=None` because its own spec says the grippers START closed on the tray. The change stayed pending for the whole run, every run. Applied on the spot now when there is nothing to arrive at; the arrival gate stays where there is. Re-recorded **0.0000 -> 0.5176 rad**, which is exactly the `grip_for(30)` the schedule asks for | `test_a_grip_with_no_object_is_not_left_pending.py` |
| **three instruments reported defects that did not exist** | `accuracy_table` mapped cubes to pads with `i % 2` -- a single-arm row -- so cubes 1 and 2 were scored against the pad **0.58 m away**: 319.7 mm placement on clips whose scene log says 56 mm. `status_table` called T1 under four modes a REAL GAP when T1 declares it runs under 06 ONLY. `verify_gripper_motion` demanded a RELEASE from a carry task that never releases. All three now read the task's own declaration. **And the fallback I wrote into the third did the same thing again**: a bare `except` around an import that failed, returning the conservative default, so the helper answered wrongly only when called from the one file that needed it | `scripts/accuracy_table.py --self-test` |
